...
path = 'data/seizure_data/train/'


def check_channels_of_interest(path, channels_of_interest = ["F3", "Fz", "F4", "C3", "Cz", "C4", "P3", "Pz", "P4"]):
...
def data_summary(directory):
    ...
    for i in [x for x in os.listdir(directory) if x.endswith('.edf')]:
        ...
        common_channels.append(set([channel.replace('EEG', '').replace('-', '').strip().upper() for channel in channels]))
...
print(data_summary('data/seizure_data/train/'))